"""
Multi-Camera Synchronized Video Recorder with DataPixx Trigger Support
No FFmpeg dependency - pure OpenCV with enhanced error handling
Sends DataPixx digital triggers on recording start (ON) and stop (OFF)
"""
...
def _dpx_send_trigger(trig: int):
    ...
    if trig == 0:
        ...
        val = 1 << (2 * trig)   # matches existing mapping
    bit_mask = (1 << 256) - 1
...
class MultiCameraRecorder:
    ...
    def __init__(self, 
                 ...
                 trigger: int = 1):
        """
        Initialize multi-camera recorder with DataPixx trigger support
        ...
        Args:
            ...
            trigger: DataPixx trigger number (1, 2, 3, etc.) sent on recording start/stop
        """
        ...
        # Open DataPixx connection
        _dpx_open()
        
        logger.info(f"MultiCameraRecorder initialized with cameras: {self.camera_ids}, trigger: {self.trigger}")
    ...
    def start_recording(self, trial_name: Optional[str] = None) -> bool:
        ...
        if success_count > 0:
            ...
            # Send DataPixx trigger ON on first frame of recording
    ...
    def stop_recording(self) -> Dict[int, Tuple[int, List[float]]]:
        ...
        # Send DataPixx trigger OFF
        logger.info(f"Sending DataPixx trigger OFF (0)")
        _dpx_send_trigger(0)
...
def multicam_recorder_DataPixx(*args, **kwargs) -> MultiCameraRecorder:
    """
    Create a recorder instance with DataPixx trigger support.
    Use with context manager for automatic cleanup.
    
    Args:
        trigger: DataPixx trigger number (1, 2, 3, etc.) - default 1
        All other args passed to MultiCameraRecorder.
    
    Example:
        with multicam_recorder_DataPixx(camera_ids=[0, 1], trigger=1) as recorder:
